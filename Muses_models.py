...
class TwoStageDetector(nn.Module):
    ...
    def forward(self, input, rois, target, reg_target, prop_type):
        ...
        transposedF = torch.transpose(input, 1, 2).cuda()
        videoF = transposedF[:, :, :1024].cuda()
        audioF = torch.zeros(input.shape[0], input.shape[2], 1024).cuda()
        audioF[:, :, :128] = transposedF[:, :, 1024:].cuda()
        input = torch.transpose(self.TBMRF_block(audioF, videoF, 1), 1, 2).cuda()
